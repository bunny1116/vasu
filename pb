...
　
            ...
            if (result.Customer != null)
            {
                ...
                if (result.Customer.IsEntity == true)
                {
                    ...
                    if (result.EntityFormationCountryRiskDesignation != null)
                        {
...
　
        ...
        public PEPChangeRequestEntity AddNewChangeRequest(PEPEntity PEP)
        {
            ...
            string[] arr = new string[] { "CreatedTS", "BookingOffice", "Customer", "PEPRequestStatu", "ServiceRequestType", "PrimarypepCountryLOB", "PrimarypepAssociatedCountryLOB", "PepRequestStatus", "CustomerStatusId", "CrNo" };
...
　
       ...
       public bool UpdateCRChangedFields(int ChangeRequestId, PEPEntity  pepdetails)
        {

             // var pepentity = GetSinglePEPbyidforCR(pepdetails.PepId, ChangeRequestId);
             Dictionary<string, List<string>> changedproperties = ComparePEPs(pepdetails.PepId, pepdetails);
            //  var changedproperties = pepentity.changedpropertiesforCR;
            //Comparing Original record to CR request...
            StringBuilder strchangeresults = new StringBuilder();
            ...
            string ChangedFields = Convert.ToString(strchangeresults);
            // update changed fields table in cr coloumn 

            var crfields = changeRequestDataAccess.FindSingleBy(cr => cr.ChangeRequestId == ChangeRequestId);
            crfields.ChangedFields = ChangedFields;
            changeRequestDataAccess.Update(crfields);
            //var user = pepdetailsdataaccess..Users.FirstOrDefault(u => u.UserId == userId);
...
　
                    ...
                    if (hasCRprimarypep)
                    {
                        ...
                        bool HasPreviousPrimaryPep = primarypepChangeStatustoComplete.Any(X => X.ChangeRequestID != null);
                        if (HasPreviousPrimaryPep)
                        {
                            var PreviousPrimaryPepApprovedData = primarypepChangeStatustoComplete.Where(x => x.PreviousPrimaryPepId == null).ToList();
                            int i = 1;
                            foreach (CDDGPrimaryPep item in PreviousPrimaryPepApprovedData)
                            {
                                string seq = Convert.ToString(PreviousPrimaryPepApprovedData.Count() + i);
                                //item.EcnSeqVal = seq;
                                //item.EcnSeqValNum = seq;
                                item.EndDate = null;
                                primarypepdataaccess.AddNew(item);
                                i++;
                            }
                        }
